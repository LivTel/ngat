 ...
 * SId$
 */
public class Polarimeter extends Instrument {
    ...
    public Polarimeter(String name) { 
 	super(name);	
	...
	PolarimeterConfig spec = (PolarimeterConfig)ic;	
	try {
	...
	try {
	    PolarimeterDetector detector = new PolarimeterDetector();	    
	    setDetector(0, detector);
	} catch (IllegalArgumentException e) {
	    System.err.println("PolarimeterSpec: "+name+" (configure) Unexpected error setting default config: "+e);
	}
    }
	...
	PolarimeterConfig   instConfig  = null;	
	PolarimeterDetector detector    = new PolarimeterDetector();
	detector.setXBin(2);
	detector.setYBin(2);
	...
	standards = new PolarimeterConfig[configCount];
	
	for (int i = 0; i < configCount; i++) {	 	 
	    instConfig  = new PolarimeterConfig("Std-"+name+"-("+i+")");
	...
	if (! (config instanceof PolarimeterConfig)) return false;
	PolarimeterConfig cc = (PolarimeterConfig)config;	
	Detector[] dets = cc.getDetectors();
	if (dets == null || (dets.length != 1)) return false;
	PolarimeterDetector det = (PolarimeterDetector)dets[0];
