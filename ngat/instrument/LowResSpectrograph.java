...
import java.io.*;
import java.text.*;
/** Stores information about a LoResSpectrograph - its current configuration and
 * methods for calculating reconfiguration times etc.
 * SId$
 */
public class LowResSpectrograph extends Instrument {
    ...
    /** Default minimum wavelength (Angs).*/
    public static final double DEFAULT_MIN_WAVELENGTH = 2500.0;

    /** Default mmaximum wavelength (Angs).*/
    public static final double DEFAULT_MAX_WAVELENGTH = 9500.0;

    /** Grating wavelength.*/
    protected double wavelength;

    protected double minWavelength = DEFAULT_MIN_WAVELENGTH;

    protected double maxWavelength = DEFAULT_MAX_WAVELENGTH;

    /** Set the grating wavelength (angstrom).*/
    public void setWavelength(double wavelength) { this. wavelength = wavelength; }

    /** Return the grating wavelength (angstrom).*/
    ...
    public LowResSpectrograph(String name) { 
 	super(name);	
	...
	LowResSpecConfig spec = (LowResSpecConfig)ic;
	setWavelength(spec.getWavelength());
	try {
	    setDetector(0, spec.getDetector(0));
	} catch (IllegalArgumentException e) {
	    System.err.println("LoResSpec: "+name+" (configure) Unexpected error setting detector from config: "+e);
	}
    }
	...
	props.load(in);
	try {
	    minWavelength = props.getDoubleValue("min.wavelength");
	} catch (ParseException px) {
	    throw new IllegalArgumentException("LowResSpec: Failed to parse min.wavelength: "+px);
	}
	try {
	    maxWavelength = props.getDoubleValue("max.wavelength");
	} catch (ParseException px) {
	    throw new IllegalArgumentException("LowResSpec: Failed to parse max.wavelength: "+px);
	}
	...
	setWavelength(0.0); 
	try {
	    LowResSpecDetector detector = new LowResSpecDetector();	    
	    setDetector(0, detector);
	} catch (IllegalArgumentException e) {
	    System.err.println("LoResSpec: "+name+" (configure) Unexpected error setting default config: "+e);
	}
    }
	...
	setWavelength(props.getDoubleValue("wavelength", 0.0));
	LowResSpecDetector detector = new LowResSpecDetector();	    
	setDetector(0, detector);
	setDetector(0, detector);
	...
	LowResSpecConfig   instConfig  = null;	
	LowResSpecDetector detector    = new LowResSpecDetector();
	detector.setXBin(1);
	detector.setYBin(1);
	...
	standards = new LowResSpecConfig[configCount];
	
	for (int i = 0; i < configCount; i++) {	 	 
	    instConfig  = new LowResSpecConfig("Std-"+name+"-("+i+")");
	    instConfig.setDetector(0, detector);
	    String strwave = props.getProperty("standard."+name+".config."+i+".wavelength");
	    if (strwave == null)
		throw new IllegalArgumentException("Standards: "+instConfig.getName()+
						   " Wavelength Not set.");
	    double wave = 0.0;
	    try {
		wave = Double.parseDouble(strwave);
	    } catch (NumberFormatException nx) {
		throw new IllegalArgumentException("Standards: "+instConfig.getName()+
						   " Wavelength format.");
	    }
	    instConfig.setWavelength(wave);
	...
	LowResSpecConfig config = new LowResSpecConfig(name);
	config.setWavelength(wavelength);	
	try {
	    config.setDetector(0, detectors[0]);
	} catch (IllegalArgumentException e) {
	    System.err.println("LowResSpec: "+name+" (getConfig) Unexpected error setting detector in config: "+e);
	}
	return config;
    }
	...
	if (! (config instanceof LowResSpecConfig)) return false;
	LowResSpecConfig cc = (LowResSpecConfig)config;
	double wavelength = cc.getWavelength();
	// Check wavelength range.
	if (wavelength < minWavelength || wavelength > maxWavelength) return false;

	Detector[] dets = cc.getDetectors();
	if (dets == null || (dets.length != 1)) return false;
	LowResSpecDetector det = (LowResSpecDetector)dets[0];
