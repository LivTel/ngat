...
/** Stores information about a TipTiltCam camera - its current configuration and
 * methods for calculating reconfiguration times etc.
 * SId$
 */
public class TipTiltCam extends Instrument implements Logging {
	...
	try {
	    ...
	    throw new IllegalArgumentException("TipCam: "+name+
    ...
    /** Load the TipTiltCam camera initial configuration from a file layed out in the
     * java.util.Properties format. As a side effect, the FilterDatabase is also
     * setup with the filters specified in the named property files.
	...
	try {
	    ...
	    throw new IllegalArgumentException("TipTiltCam: "+name+
					       " (configure) Error parsing filterwheel.count : "+ne);
	}
	infoLog.log(INFO, 2, "TipTiltCam", name, 
	...
	for (int wheel = 0; wheel < wheelCount; wheel++) {
		...
		System.err.println("TipTiltCam::Makefilter from: "+dbFile.getPath()+", for filter type: "+type+" id: "+id);
		
		filter = makeFilter(type, id, dbFile);
		infoLog.log(INFO, 2, "TipTiltCam", name, 
			    "Configured TipTiltCam for filter: "+filter);
	...
	standards = new THORConfig[configCount];
	
	for (int i = 0; i < configCount; i++) {	 	 
	    instConfig  = new THORConfig("Std-"+name+"-("+i+")");
	    ...
	    instConfig.setFilter(usefilter);
	...
	try {	 
	    ...
	    infoLog.log(INFO, 2, "TipTiltCam", name, "config(V)", "Filter:" + usefilter);
	    setFilter(usefilter.getType());
	    THORDetector detector = new THORDetector();	    
	    setDetector(0, detector);
	} catch (IllegalArgumentException e) {
	    System.err.println("TipTiltCam: "+name+" (configure) Unexpected error setting default config: "+e);
	}
    }
	...
	setFilter(props.getProperty("filter", THORConfig.BLANK));	
	THORDetector detector = new THORDetector();	    
	setDetector(0, detector);
	...
	THORConfig irc = (THORConfig)ic;
	setFilter(irc.getFilter());	
	try {
	    setDetector(0, irc.getDetector(0));
	} catch (IllegalArgumentException e) {
	    System.err.println("TipTiltCam: "+name+" (configure) Unexpected error setting detector from config: "+e);
	}
    }
	...
	THORConfig config = new THORConfig(name);
	config.setFilter(filter.getType());	
	try {
	    config.setDetector(0, detectors[0]);
	} catch (IllegalArgumentException e) {
	    System.err.println("TipTiltCam: "+name+" (getConfig) Unexpected error setting detector in config: "+e);
	}
	return config;
    }
	...
	if (! (config instanceof THORConfig)) return false;
	THORConfig cc = (THORConfig)config;
	String usefilter = cc.getFilter();
	...
	THORDetector det = (THORDetector)dets[0];
    ...
    /** Calculates and returns the readout time for this TipTiltCam in
	...
	props.load(in);
	System.err.println("TipTiltCam::Loaded filter DB: "+propsFile.getPath());
