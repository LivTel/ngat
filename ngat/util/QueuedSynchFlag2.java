 ...
 * </pre>
 *
 * $Id: QueuedSynchFlag2.java,v 1.1 2002-09-25 10:48:55 snf Exp $
 *
 */
public class QueuedSynchFlag2 extends SynchFlag {
    ...
    public QueuedSynchFlag2(int size) {
	super();
    ...
    public synchronized void getSynchFlag(int type) throws InterruptedException {
     ...
     * correctly.
     * @param type The Thread can be READER, WRITER or OBSERVER. */
    public synchronized void getSynchFlag(int type, long timeout) throws InterruptedException {

	Thread caller = Thread.currentThread();
	System.err.println("############## QSF(2): Caller is: "+caller+" Flag is: "+flag);
	
	if (caller == flag) { // already got lock..
	    count++;
	    return;
	}

	if (type == READER) {
	    waiters.insertElementAt(caller, 0); // put the reader first.
	    serverIsAttached = true;
	} else {
	    if (type == WRITER || type == OBSERVER){
		waiters.addElement(caller); // put writers in arrival order.
	    }
	}
	...
	System.err.println("############## QSF(2): About to WAIT("+timeout+")");
	while ((Thread)waiters.elementAt(0) != caller)
	    wait(timeout); // The calling thread [me] waits..
	System.err.println("############## QSF(2): Done WAIT("+timeout+")");
	
	// Entered only when notifyAll() has been called by the current owner.
	// and [me] has reached head of queue..

	flag = caller; // Actually make [me] the flag owner..
	count = 0; // Set the count (for nested calls)..
    }
...
/** Revision 1.1  2000/11/21 16:52:11  snf
/** Initial revision
/** */
