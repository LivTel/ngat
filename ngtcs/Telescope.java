...
public class Telescope
{
   ...
   * Rotator mechanism.
   */
  protected Rotator rotator = null;
  ...
  public double MAX_TRACKING_ERROR;
  ...
  public void initialise() throws InitialisationException
  {
    ...
    System.err.println( "Logging initialised" );
    ...
    logger.log( 1, logName, "Initialising Mount ["+mountClassName+"]" );
    ...
    try
    {
      ...
      MAX_TRACKING_ERROR = Math.toRadians( mteArcsecs / 3600.0 );
    }
    catch( NGATPropertyException npe )
    {
      logger.log( 1, logName, npe );
      InitialisationException ie = new InitialisationException( npe );
      throw ie;
    }
    /*********************************************************************/
    ...
    {
      logger.log( 1, logName, "Using null Rotator" );
    }
    else
    {
      logger.log( 1, logName, "Initialising Rotator ["+
		  rotatorClassName+"]" );
      rotator = (ngat.ngtcs.subsystem.Rotator)
	instantiateByName( rotatorClassName );
    ...
    */
    ...
    logger.log( 1, logName, "Initialising Timer ["+timerClassName+"]" );
    ...
    logger.log( 1, logName, "Initialising AstrometryCalculator ["+
		astroCalcClassName+"]" );
    try
    {
      astroCalc = (ngat.ngtcs.AstrometryCalculator)
      ( Class.forName( astroCalcClassName ).newInstance() );
    }
    catch( Exception e )
    {	    
      throw new InitialisationException
	( "couldn't instantiate "+astroCalcClassName+" : "+e );
    }
    ...
    logger.log( 1, logName, "Initialising Instrument Ports" );
    ...
    while( morePorts )
    {
      ...
      {
	logger.log
	  ( 1, logName, "null String returned from port"+nPorts+
	...
	try
	{
	  ...
	  logger.log
	    ( 3, logName, "Configuring VirtualTelescope ["+n+"] with:"+
	      ...
	      "\n     instrument centre (Y) = "+y+
	      "\ninstrument alignment angle = "+a+
	      "\n               1 arcsecond = "+(1.0/s)+" "+u+"s" );
	}
	catch( NoSuchElementException nsee )
	{
	  logger.log( 1, logName, "incomplete port descriptor on "+
		      "port ["+nPorts+"] : "+nsee );
	}
	catch( NumberFormatException nfe )
	{
	  logger.log( 1, logName, "cannot parse instrument port ["+nPorts+
		      "] descriptor ["+portDesc+"] : "+nfe );
	}
	...
	if( goodDesc )
	{
	  ...
	  Counter c = (Counter)( portNameCounts.get( n ) );
	  c.increment();
	  ...
	  FocalStation fs = new FocalStation
	    ( fsName, p, f, new PointingOrigin( x, y  ), a, s, u );
	  focalStationList.add( fs );
	  /* Instantiate the VirtualTelescope */
	  VirtualTelescope vt = 
	    new VirtualTelescope( this, fs, astroCalc, focalLength );
	  virtualTelescopeList.add( fsName );
	  virtualTelescopeHash.put( fsName, vt );

	  logger.log
	    ( 1, logName, "VirtualTelescope ["+fsName+"] configured" );
	}
      }
      nPorts++;
    }
    ...
    String s = new String( virtualTelescopeList.size()+
			   " Instrument Ports configured : " );
    for( int i = 0; i < virtualTelescopeList.size(); i++ )
    {
      s = new String( s+" "+virtualTelescopeList.get( i ) );
    }
    ...
    /*********************************************************************/
    logger.log( 1, logName, "Initialising PluggableSubSystems" );
  ...
  /**
   * Instantiate a class by using its Factory class.
   * <p>
   * This method first instantiates a <i>InputArgumentClass</i>Factory using
   * the blank constructor (<code>newInstance()</code>) and then calls the
   * <i>PluggableSubSystemCreator</i>-implementing <code>getInstance</code> to
   * return either a newly instantiated Class, or the single instance Singleton
   * of the Class, as defined in the Factory.
   */
    ...
    throws InitialisationException
  {
    ...
    {
      throw new InitialisationException
	( "couldn't instantiate mechanism : "+
	  "input mechanism class name is null or \"\"" );
    }
	...
	throw new InitialisationException
	  ( "couldn't instantiate mechanism : "+facClazz+
  ...
  /**
   * Return the Rotator used by this Telescope.
   * @return this Telescope's Rotator
   */
